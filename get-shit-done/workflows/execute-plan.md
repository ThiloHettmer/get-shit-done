...
Read agent type for subagent spawning (default: gsd-executor):

```bash
AGENT_TYPE=$(cat .planning/config.json 2>/dev/null | grep -o '"agent_type"[[:space:]]*:[[:space:]]*"[^"]*"' | grep -o '"[^"]*"$' | tr -d '"' || echo "gsd-executor")
```

**Resolve Executor Model:**

1. Check for explicit `executor_model` in config.
2. If not set, use `model_profile` to map to default models.

```bash

CONFIG_MODEL=$(cat .planning/config.json 2>/dev/null | grep -o '"executor_model"[[:space:]]*:[[:space:]]*"[^"]*"' | grep -o '"[^"]*"$' | tr -d '"')

if [ -n "$CONFIG_MODEL" ]; then
  EXECUTOR_MODEL="$CONFIG_MODEL"
else
  # Fallback to profile mapping
  case "$MODEL_PROFILE" in
    "quality") EXECUTOR_MODEL="claude-3-5-opus-latest" ;;
    "balanced") EXECUTOR_MODEL="claude-3-5-sonnet-latest" ;;
    "budget") EXECUTOR_MODEL="claude-3-5-sonnet-latest" ;;1
    *) EXECUTOR_MODEL="claude-3-5-sonnet-latest" ;;
  esac
fi
```

Store `EXECUTOR_MODEL` and used it as the `model` parameter in Task calls.
...
**Special Pattern: Granular Mode (For gsd-granular agent)**

If `AGENT_TYPE` is "gsd-granular":

- **Force segmentation:** Every single task becomes its own autonomous segment.
- **Ignore Plan Structure:** Even if the plan has no checkpoints, we treat it as a segmented plan.
- **Micro-batching:** Task 1 -> Subagent -> Commit -> Task 2 -> Subagent -> Commit...

**Standard Patterns (For standard gsd-executor):**
...
2. Use Task tool with subagent_type="{agent_type}" and model="{EXECUTOR_MODEL}":
...
**Logic to determine execution path:**

```bash
if [ "$AGENT_TYPE" = "gsd-granular" ]; then
   # FORCE PATTERN B (Segmented) logic, but with virtual checkpoints
   echo "Granular agent detected. Forcing segmented execution."
   EXECUTION_MODE="segmented"
elif [ -z "$CHECKPOINTS" ]; then
...
1. **Segmented plans** (Pattern B: has checkpoints)
2. **Granular Mode** (Pattern D: forced segmentation for gsd-granular)

For Pattern A (fully autonomous) and Pattern C (decision-dependent) with standard agent, skip this step.
...
2. For each segment in order:
   ...
   B. If routing = Subagent:
      ```
      Spawn Task tool with subagent_type="{agent_type}" and model="{EXECUTOR_MODEL}":
